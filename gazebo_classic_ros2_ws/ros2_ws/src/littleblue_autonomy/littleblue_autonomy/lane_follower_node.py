...
KNOWN_OBSTACLES = [
    ...
    # Ramp (radius set to 0 so the robot doesn't avoid it)
    (2.0, 0.0, 0.0, 'ramp_1'),
]
...
def _build_track_centerline():
    """Centerline for the Oval Track.
    Straight length = 12 (x from -6 to 6).
    Curves centered at (±6, 5) with radius 5.
    """
    pts = []
    step = 0.25
    R = 5.0
    n_arc = int(math.pi * R / step)
    ...
    # 2. Right curve: center (6, 5)
    cx, cy = 6.0, 5.0
    for i in range(n_arc + 1):
        theta = -math.pi / 2 + math.pi * i / n_arc
        pts.append((cx + R * math.cos(theta), cy + R * math.sin(theta)))

    # 3. Top straight: y=10, x from 6 to -6
    x = 6.0
    while x >= -6.0:
        pts.append((x, 10.0))
        x -= step

    # 4. Left curve: center (-6, 5)
    cx, cy = -6.0, 5.0
    for i in range(n_arc + 1):
        theta = math.pi / 2 + math.pi * i / n_arc
        pts.append((cx + R * math.cos(theta), cy + R * math.sin(theta)))

    # الروبوت بيمشي عكس عقارب الساعة (CCW) فمش هنعمل reverse للمصفوفة
    return pts
...
class LaneFollowerNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('lane_width', 2.0)
...
# ─── clearance computation ─────────────────────────────────────
    ...
    def _reconstruct_lane_walls(self):
        ...
        def ransac_fit(points, n_iter=100, thresh=0.40):
            if len(points) < 4:
                return None
    ...
    def _extract_lane_boundaries(self):
        ...
        for px, py in self.lane_points:
            if min_x <= px < max_x:
                i = int((px - min_x) / bin_w)
                if 0 <= i < n_bins:
                    bins[i].append(py)
        ...
        for i in range(n_bins):
            ...
            # تجميع النقط الذكي بدل RANSAC الغبي
            pts_y = sorted(bins[i])
            clusters = []
            curr = [pts_y[0]]
            for y in pts_y[1:]:
            ...
            centers.sort(reverse=True)  # الشمال (الموجب) الأول، وبعدين اليمين (السالب)
            
            if len(centers) == 2:
                lane_boundaries[i] = (centers[0], centers[1])
            elif len(centers) == 1:
                cy = centers[0]
                if cy > 0:
                    lane_boundaries[i] = (cy, cy - 2.0)  # عرض التراك بتاعك 2 متر
                else:
                    lane_boundaries[i] = (cy + 2.0, cy)
                    
            if i in lane_boundaries:
                ly, ry = lane_boundaries[i]
                # بناء جدران ناعمة مع الكيرف مش بتقطعه
                self._lane_wall_obstacles.append((min_x + (i + 0.5) * bin_w, ly + 0.2, 0.05))
                self._lane_wall_obstacles.append((min_x + (i + 0.5) * bin_w, ry - 0.2, 0.05))

        # ملء الفراغات لو الكاميرا مشوشة
        for i in range(n_bins):
            if i not in lane_boundaries:
                for offset in (1, -1, 2, -2, 3, -3):
                    if i + offset in lane_boundaries:
                        lane_boundaries[i] = lane_boundaries[i + offset]
                        break
    ...
    def _plan_path_legacy(self):
        ...
        if not lane_boundaries:
            self.get_logger().warn('PLANNER FAILED: RANSAC rejected the lines!')
            return None
        ...
        self.get_logger().warn('PLANNER FAILED: A* blocked by obstacles or no path found!')
        return None
    ...
    def _control_loop(self):
        ...
        if n_lane < 10 and lane_age < 5.0: #n_lane < 100
            if self._log_counter % 20 == 0:
        ...
        if path and len(path) >= 2:
            ...
            if result is not None:
                ...
                if self.obstacle_points:
                    min_dist = 999.0
                    for ox, oy, _ in self.obstacle_points:
                        dist = math.sqrt(ox * ox + oy * oy)
                        if dist < min_dist:
                            min_dist = dist
                        if dist < 2.5 and ox > 0.1 and abs(oy) < 1.5:
                            repulsive_az += (-oy) / max(dist * dist, 0.1)

                    repulsive_az = max(-1.5, min(1.5, repulsive_az * 0.8))
                    cmd.angular.z += repulsive_az
